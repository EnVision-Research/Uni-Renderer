...
def get_up_block(
    ...
    elif up_block_type == "UpResBlock2D":
        return UpResBlock2D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
    ...
    elif up_block_type == "CrossAttnUpResBlock2D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpResBlock2D")
        return CrossAttnUpResBlock2D(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
class CrossAttnUpResBlock2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        hidden_states: torch.FloatTensor,
        res_hidden_states_tuple: Tuple[torch.FloatTensor, ...],
        up_additional_states_tuple: Tuple[torch.FloatTensor, ...],
        temb: Optional[torch.FloatTensor] = None,
        encoder_hidden_states: Optional[torch.FloatTensor] = None,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        upsample_size: Optional[int] = None,
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            up_additional_states = up_additional_states_tuple[0]
            up_additional_states_tuple = up_additional_states_tuple[1:]
            ...
            hidden_states += up_additional_states
            output_states += (hidden_states, )
...
class UpBlock2D(nn.Module):
    ...
    def forward(
        ...
        output_states = () # Modification: add residual states
