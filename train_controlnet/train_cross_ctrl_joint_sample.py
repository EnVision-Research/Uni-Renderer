...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, controldec, args, accelerator, weight_dtype, step):
    ...
    for validation_prompt in args.validation_prompt:
        ...
        image_logs.append(
            {"images": images, "masks_1": masks_1, "masks_2": masks_2, "validation_prompt": validation_prompt}
        )
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                tracker.writer.add_images(f"validation/{validation_prompt}", formatted_out, step, dataformats="NHWC")
                
                n,h,w,c = formatted_out.shape
                formatted_out = formatted_out.transpose(1,0,2,3).reshape(h,n*w, c)
                im = Image.fromarray(formatted_out)
                im.save(f'{save_img_dir}/{validation_prompt}{str(step)}.png')
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetVAENoImgResOneCtlModel.from_unet(unet, load_weights_from_unet=False) #zhifei
        controldec = UnetDecControlModel.from_unet(unet, load_weights_from_unet=False)
    ...
    #train_dataset = Fill50k(args.train_data_dir)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet, controldec, unet):
                #masks, images, prompts = batch
                images = batch['img']
                masks = batch['segmentation'] #[zhifei]
                depths = batch['depth']
                normals = batch['normal']
                prompts = ' '

                #print(images.shape, masks.shape, depths.shape, normals.shape)
                ...
                bsz = latents_img.shape[0]

                # Sample a random timestep for each image
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_img.device)
                timesteps = timesteps.long()

                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                noisy_latents_img = noise_scheduler.add_noise(latents_img, noise_img, timesteps)

                # ------------------- mask -----------------------
                # Convert mask to latent space
                latents_mask = vae.encode(masks.to(dtype=weight_dtype)).latent_dist.sample()
                latents_normal= vae.encode(normals.to(dtype=weight_dtype)).latent_dist.sample()# zhifei 
                latents_mask = latents_mask * vae.config.scaling_factor
                latents_normal = latents_normal * vae.config.scaling_factor # zhifei 

                latents_masks_normal = torch.cat((latents_mask, latents_normal), dim=1) # zhifei 
                #print(latents_masks_normal.shape)
                # Sample noise that we'll add to the latents
                #noise_mask = torch.randn_like(latents_mask) # 4*64*64
                noise_mask = torch.randn_like(latents_masks_normal) #zhifei 8 x 64 x 64
                bsz = latents_mask.shape[0]

                # Sample a random timestep for each mask   # disabled in mask branch
                # timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_mask.device)
                # timesteps = timesteps.long()

                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                noisy_latents_mask = noise_scheduler.add_noise(latents_masks_normal, noise_mask, timesteps) #zhifei
                ...
                down_block_res_samples, mid_block_res_sample, raw_down_block_res_samples_ctlnet, raw_mid_block_sample_ctlnet = controlnet(
                    ...
                    controlnet_cond=noisy_latents_mask,
                    return_dict=False,
                )
