...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, controldec, args, accelerator, weight_dtype, step):
    ...
    pipeline.scheduler_normal = UniPCMultistepScheduler.from_config(pipeline.scheduler.config) # TODO: incorprate with more future modalities
    ...
    image_logs = []
    validation_imgs = []
    for img in args.validation_image:
        validation_img = Image.open(img).convert("RGB").resize((256,256))
        validation_imgs.append(validation_img)
    #validation_mask = Image.open(args.validation_image[1]).convert("RGB")
    ...
    for val_img in validation_imgs:
        with torch.autocast("cuda"):
                # image, mask_1, mask_2= pipeline(
                #     [""], # None prompt
                #     None, # None image
                #     height=args.resolution, width=args.resolution, num_inference_steps=20, generator=generator
                # )
                mask_1, mask_2 = pipeline.image2mask(
                    [""], # None prompt
                    val_img, # None image
                    height=args.resolution, width=args.resolution, num_inference_steps=20, generator=generator
                )
        images.append(val_img)
        masks_1.append(mask_1[0])
        masks_2.append(mask_2[0])

    image_logs.append(
            {"images": images, "masks_1": masks_1, "masks_2": masks_2}
        )
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                masks_first = log["masks_1"] # zhifei
                masks_second = log["masks_2"]
                formatted_masks_1 = []
                formatted_masks_2 = []
                for mask in masks_first:
                    formatted_masks_1.append(np.asarray(mask))
                formatted_masks_1 = np.stack(formatted_masks_1)
                for mask in masks_second:
                    formatted_masks_2.append(np.asarray(mask))
                formatted_masks_2 = np.stack(formatted_masks_2)
                #print(formatted_masks_1.shape, formatted_masks_2.shape, formatted_images.shape)

                formatted_out = np.concatenate([formatted_images, formatted_masks_1, formatted_masks_2], axis=1)
...
def main(args):
    ...
    train_dataset = NYUv2(args.train_data_dir, 256)
    task_list = ['segmentation', 'normal'] #TODO: depth has 1 channel, how to pass it to vae that requires 3 channel input
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=True,
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetVAENoImgResOneCtlModel.from_unet(unet, load_weights_from_unet=False, len_t=len(task_list)) #zhifei exclude img as we dont pass it to controlnet
        controldec = UnetDecControlModel.from_unet(unet, load_weights_from_unet=False, len_t=len(task_list))
