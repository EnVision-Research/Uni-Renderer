...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, 
    controldec, args, accelerator, weight_dtype, step, img_guidance_scale=7.5, compute_times=1, mask_guidance_scale=7.5):
    ...
    images, real_images = [], []
    metallics, roughnesses, masks, normals, albedos, lights = [], [], [], [], [], []
    for index in tqdm(range(args.num_validation_images), desc='infer'):
        ...
        albedo_image = Image.open(
            os.path.join(test_data_path, f'albedo_{index}.png')
            ).convert("RGB").resize((args.resolution, args.resolution))
        # light_image = Image.open(
        ...
        with torch.autocast("cuda"):
            ...
            for _ in range(compute_times):
                material, normal, albedo, light = pipeline.image2mask_3mod_albedo(
                    [""],
                    validation_img, 
            ...
            rgb_image = pipeline.mask2image_3mod_albedo(  # tianshuo
                [""],
                material_image,
                normal_image, 
                albedo_image,
            ...
            for log in image_logs:
                ...
                formatted_out = np.concatenate(
                    [formatted_metallics, formatted_roughnesses, formatted_masks, 
                     formatted_normals, formatted_albedos, formatted_lights, formatted_images, formatted_real_images], axis=1)
                ...
                im.save(f'{save_img_dir}/{str(step)}_imcfg{img_guidance_scale}_attrcfg{mask_guidance_scale}_times{compute_times}.png')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet, controldec, unet):
                ...
                latents_attr = torch.cat((latents_material, latents_normal, latents_albedo, latents_light), dim=1)
