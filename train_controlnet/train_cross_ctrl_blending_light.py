...
from dataset.blendGen import BlenderGenDataset, BlenderGenDataset_old
...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, controldec, args, accelerator, weight_dtype, step):
    ...
    metallic_image = Image.open(args.validation_image[1]).convert("RGB").resize((args.resolution, args.resolution))
    roughness_image = Image.open(args.validation_image[2]).convert("RGB").resize((args.resolution, args.resolution))
    normal_image = Image.open(args.validation_image[3]).convert("RGB").resize((args.resolution, args.resolution))
    light_image = Image.open(args.validation_image[4]).convert("RGB").resize((args.resolution, args.resolution))
    
    images = []
    metallics, roughnesses, normals, lights = [], [], [], []
    for _ in range(args.num_validation_images):
            with torch.autocast("cuda"):
                metallic, roughness, normal, light = pipeline.image2mask(
                    [""],
                    validation_img, 
                    height=args.resolution, width=args.resolution, num_inference_steps=50, generator=generator
                )
                rgb_image = pipeline.mask2image(
                    [""],
                    metallic_image,
                    roughness_image,
                    normal_image, 
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                formatted_out = np.concatenate([formatted_metallics, formatted_roughnesses, formatted_normals, formatted_lights, formatted_images], axis=1)
...
def main(args):
    ...
    task_list = ['img', 'metallic_image', 'roughness_image', 'normal', 'light'] #TODO: depth has 1 channel, how to pass it to vae that requires 3 channel input

    #train_dataset = BlenderGenDataset_old(root_dir=args.train_data_dir, mode='train', transform=transforms, resize=(256, 256))
    train_dataset = BlenderGenDataset_old(root_dir=args.train_data_dir, mode='train', random_flip=False, random_rotation=False, resize=(args.resolution, args.resolution))
    ...
    controlnet.conv_in.weight = nn.Parameter(controlnet.conv_in.weight.repeat(1, 4, 1, 1) * 0.25)
    controlnet.time_embedding.linear_1.weight = nn.Parameter(controlnet.time_embedding.linear_1.weight.repeat(1, 4) * 0.25)
    ...
    controlnet.config["in_channels"] = 16

    # Replace the last layer to output 8 out_channels. 
    controldec.conv_out.weight = nn.Parameter(controldec.conv_out.weight.repeat(4, 1, 1, 1) * 0.25)
    controldec.conv_out.bias = nn.Parameter(controldec.conv_out.bias.repeat(4) * 0.25)
    controldec.time_embedding.linear_1.weight = nn.Parameter(controldec.time_embedding.linear_1.weight.repeat(1, 4) * 0.25)
    config_dict = {}
    for k,v in controldec._internal_dict.items():
        config_dict[k] = v
    config_dict = EasyDict(config_dict)
    controlnet._internal_dict = config_dict
    controldec.config["out_channels"] = 16
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet, controldec, unet):
                ...
                #mask_image = batch['rgb']
                metallic_image = batch['metallic']
                roughness_image = batch['roughness']
                ...
                timesteps = compute_t(len_t=5, 
                                  num_timesteps=noise_scheduler.config.num_train_timesteps, 
                                  bs=bsz, 
                                  device=images.device)
                
                timesteps_img, timesteps_metallic, timesteps_roughness, timesteps_normal, timesteps_light = timesteps[0], timesteps[1], timesteps[2], timesteps[3], timesteps[4]
                ...
                noise_light = torch.randn_like(latents_light)
                noisy_latents_light = noise_scheduler.add_noise(latents_light, noise_light, timesteps_light)
                ...
                timestep_label_concat = torch.cat((timesteps_metallic.unsqueeze(0), timesteps_roughness.unsqueeze(0), timesteps_normal.unsqueeze(0), timesteps_light.unsqueeze(0)), dim=0)
                noise_concat = torch.cat((noise_metallic, noise_roughness, noise_normal, noise_light), dim=1)
                noisy_latents_concat = torch.cat((noisy_latents_metallic, noisy_latents_roughness, noisy_latents_normal, noisy_latents_light), dim=1)
