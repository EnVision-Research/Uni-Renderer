...
from dataset import Fill50k, NYUv2
from utils_metrics.compute_t import compute_t
from zf_alchemist.diffusion_4_perception.models.controlnet_old import ControlNetVAENoImgResOneCtlModel, UnetDecControlModel, UNet2DConditionModel
from zf_alchemist.diffusion_4_perception.models.pipeline_new_d4p import StableDiffusionControl2BranchFtudecPipeline
...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, controldec, args, accelerator, weight_dtype, step, test_dataloader, test_dataset, segmetric, normalmetric):
    ...
    pipeline.scheduler_mask = UniPCMultistepScheduler.from_config(pipeline.scheduler.config)
    ...
    for _ in range(args.num_validation_images):
        with torch.autocast("cuda"):
            mask, normal, _ = pipeline.image2mask(
                [""],
                validation_img, 
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                formatted_out = np.concatenate([formatted_masks,formatted_normals, formatted_j_images, formatted_j_masks, formatted_j_normals], axis=1)
...
def main(args):
    ...
    train_dataset = NYUv2(args.train_data_dir, args.resolution)
    task_list = ['img', 'segmentation', 'normal'] #TODO: depth has 1 channel, how to pass it to vae that requires 3 channel input
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=True,
    ...
    test_dataset = NYUv2(args.train_data_dir, args.resolution, mask2rgb=False, mode="test") # for testing
    ...
    segmetric = SegMetric(test_dataset.num_class+1)
    ...
    if args.controlnet_model_name_or_path:
        ...
        controldec = UnetDecControlModel.from_pretrained(args.controlnet_model_name_or_path, subfolder="controldec")
    else:
        ...
        controldec = UnetDecControlModel.from_unet(unet, len_t=len(task_list)-1)
...
# add pretrain stuff ###
    ...
    _weight_conv_in = _weight_conv_in.repeat(1, 2, 1, 1) 
    _weight_conv_in *= 0.5
    _bias_conv_in *= 0.5
    ...
    _weight_time_embedding = _weight_time_embedding.repeat(1, 2)
    _weight_time_embedding *= 0.5
    _bias_time_embedding *= 0.5
    ...
    controlnet.config["in_channels"] = 8
    ...
    _weight_conv_out = _weight_conv_out.repeat(2, 1, 1, 1) 
    _bias_conv_out = _bias_conv_out.repeat(2)
    ...
    _weight_time_embedding_dec = _weight_time_embedding_dec.repeat(1, 2)
    _weight_time_embedding_dec *= 0.5
    _bias_time_embedding_dec *= 0.5

    # new conv_out channel
    _n_convout_out_channel = controldec.conv_out.out_channels
    _new_conv_out = nn.Conv2d(
        _n_convout_out_channel, 8, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
    )
    ...
    config_dict = EasyDict(config_dict)
    controlnet._internal_dict = config_dict
    controldec.config["out_channels"] = 8
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    ...
                    elif model.__class__.__name__ == 'UnetDecControlModel':
                        sub_dir = "controldec"
                        model.save_pretrained(os.path.join(output_dir, sub_dir))
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                if model.__class__.__name__ == 'ControlNetVAENoImgResOneCtlModel':
                    ...
                    load_model = UnetDecControlModel.from_pretrained(input_dir, subfolder=subfolder)
    ...
    controlnet, controldec, unet, optimizer, train_dataloader, test_dataloader, lr_scheduler = accelerator.prepare(
        controlnet, controldec, unet, optimizer, train_dataloader, test_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet, controldec, unet):
                images, label = batch
                masks = label['segmentation']
                normals = label['normal']
                prompts = ' '

                bsz = images.shape[0]

                #prepare time step for small tasks space
                timesteps = compute_t(len_t=len(task_list), 
                                  num_timesteps=noise_scheduler.config.num_train_timesteps, 
                                  bs=bsz, 
                                  device=images.device)
                
                timesteps_img, timesteps_mask, timesteps_normal = timesteps[0], timesteps[1], timesteps[2]

                #### img ####
                latents_img = vae.encode(images.to(dtype=weight_dtype)).latent_dist.sample()
                latents_img = latents_img * vae.config.scaling_factor
                noise_img = torch.randn_like(latents_img) # 4*64*64
                #timesteps_img = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_img.device)
                #timesteps_img = timesteps_img.long()
                noisy_latents_img = noise_scheduler.add_noise(latents_img, noise_img, timesteps_img)

                #### seg ####
                latents_mask = vae.encode(masks.to(dtype=weight_dtype)).latent_dist.sample()
                latents_mask = latents_mask * vae.config.scaling_factor
                noise_mask = torch.randn_like(latents_mask)
                #timesteps_mask =  torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_mask.device)
                #timesteps_mask = timesteps_mask.long()
                noisy_latents_mask = noise_scheduler.add_noise(latents_mask, noise_mask, timesteps_mask)
                ...
                #timesteps_normal =  torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents_normal.device)
                #timesteps_normal = timesteps_normal.long()
                noisy_latents_normal = noise_scheduler.add_noise(latents_normal, noise_normal, timesteps_normal)


                #### concat label ####
                
                timestep_label_concat = torch.cat((timesteps_mask.unsqueeze(0), timesteps_normal.unsqueeze(0)), dim=0)
                noise_concat = torch.cat((noise_mask, noise_normal), dim=1)
                noisy_latents_concat = torch.cat((noisy_latents_mask, noisy_latents_normal), dim=1)


                # timesteps_label = compute_t(len_t=len(task_list), # support multi-timestepd
                #                   num_timesteps=noise_scheduler.config.num_train_timesteps, 
                #                   bs=bsz, 
                #                   device=batch["img"].device)
                
                #print(timesteps_label.shape)

                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                # latents_clean_label, noise, noisy_images = [], [], [] # latents_clean_label store all the labels latens(except for image), so does for nosie and noisy_images
                # for tidx, tname in enumerate(task_list):
                #     t_clean_image = batch[tname].to(weight_dtype) # extract img/seg/noraml
                #     t_latents_img = vae.encode(t_clean_image.to(dtype=weight_dtype)).latent_dist.sample() # convert to latents using VAE
                #     t_latents_img = t_latents_img * vae.config.scaling_factor
                #     t_noise = torch.randn_like(t_latents_img) #sample noise
                #     t_noisy_images = noise_scheduler.add_noise(t_latents_img, t_noise, timesteps_label[tidx]) # add different noise

                #     latents_clean_label.append(t_latents_img) # for all the labels have them concat
                #     noise.append(t_noise)
                #     noisy_images.append(t_noisy_images)

                # latents_clean_label = torch.cat(latents_clean_label, dim=1)
                # noise = torch.cat(noise, dim=1)
                # noisy_images = torch.cat(noisy_images, dim=1)

                #print('img', latents_img.shape, noise_img.shape, noisy_latents_img.shape)
                #dprint('label', latents_clean_label.shape, noise.shape, noisy_images.shape)
