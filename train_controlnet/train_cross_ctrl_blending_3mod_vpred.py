...
from dataset.blendGen import BlenderGenDataset, BlenderGenDataset_old, BlenderGenDataset_3mod
...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, 
    ...
    for index in range(1, args.num_validation_images+1):
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            logs = {"loss": loss.detach().item(), "loss_mask": loss_mask.detach().item(), "lr": optimizer.param_groups[0]['lr']}
