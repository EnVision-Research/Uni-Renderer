...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, 
    ...
    with torch.autocast("cuda"):
        ...
        roughness_image_path = os.path.join(args.output_dir, 'roughness', f'{base_filename}.png')
        roughness_image.save(roughness_image_path)

        albedo_image_path = os.path.join(args.output_dir, 'albedo', f'{base_filename}.png')
        albedo[0].save(albedo_image_path)

        normal_image_path = os.path.join(args.output_dir, 'normal', f'{base_filename}.png')
        normal[0].save(normal_image_path)

        specular_image_path = os.path.join(args.output_dir, 'specular', f'{base_filename}.png')
        spec_light[0].save(specular_image_path)

        diffuse_image_path = os.path.join(args.output_dir, 'diffuse', f'{base_filename}.png')
