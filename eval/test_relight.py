...
from dataset.objaverse import ObjaverseData, ObjaverseData_overfit_5, ObjaverseData_relight
...
def process_material_and_mask(material, mask_image, device):
    ...
    metallic_init = torch.zeros(batch_size, height, width, 1, dtype=torch.float32, device=device)
    roughness_init = torch.zeros(batch_size, height, width, 1, dtype=torch.float32, device=device)
...
def collate_fn(batch, single_obj=True):
    ...
    batch_size = batch['input_view_num'][0] * 2 ######## chnage back zhifei
    #batch_size = batch['input_view_num'].item()
    ...
    if not single_obj:
        ...
        for i in range(batch_size):
            ...
            pose_list_sample = batch['pose_list'][i][0]  # mvp
            camera_pos_sample = batch['camera_pos'][i][0]  # campos, mv.inverse
            c2w_list_sample = batch['c2w_list'][i]  


            pose_list_sample_2_save = batch['pose_list']  # mvp
            camera_pos_sample_2_save = batch['camera_pos'] # campos, mv.inverse
            c2w_list_sample_2_save = batch['c2w_list'] # mv
            
            spec_env = batch['env_list'][i][0] #[0][i]
            spec_env_ = []
            for k in range(len(spec_env)):
                spec_env_.append(spec_env[k][0])
            diff_env = batch['env_list'][i][1][0]
       

            env_list_sample = [spec_env_, diff_env]##batch['env_list'][0][i]  
            #material_list_sample = batch['material_list'][0][i]   
            metallic =  batch['material_list'][i][0].cuda().type(torch.float32)
            roughness = batch['material_list'][i][1].cuda().type(torch.float32)
         
            material_list_sample = [metallic, roughness]
            camera_embeddings = batch['camera_embedding_list'][i] 
    ...
    data = {
        ...
        'input_pose_list_sample_2_save': torch.stack(pose_list_sample_2_save, dim=0).detach(),
        'camera_pos_sample_2_save': torch.stack(camera_pos_sample_2_save, dim=0).detach(),
        'c2w_list_sample_2_save': torch.stack(c2w_list_sample_2_save, dim=0).detach(),
...
def save_tensor_as_image(tensor, filename, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]):
    """Save a PyTorch tensor as an image file after unnormalizing, without borders."""
    #if filename != "mask_image":
    #breakpoint()
    ...
    if npimg.shape[0] == 1:
        npimg = npimg.squeeze(0)  # Grayscale image
        npimg = np.clip(npimg, 0, 1) * 255  # Scale to [0, 255]
        npimg = npimg.astype(np.uint8)
        img = Image.fromarray(npimg, mode='L')
    else:
        npimg = np.transpose(npimg, (1, 2, 0))  # Convert to H x W x C for RGB
        npimg = np.clip(npimg, 0, 1) * 255  # Scale to [0, 255]
        npimg = npimg.astype(np.uint8)
    ...
    img.save(filename)
import json
def vis_dataset(rgb_gt, mask_image, env, normal_image,albedo_image,spec_light_image,diff_light_image, mv, mvp, campos, ind):
    root_folder = '/hpc2hdd/home/zchen379/sd3/diff_rendering_unet/dataset/other_intrinsics_gt'
    rgb_raw = rgb_gt
    rgb_images = rgb_gt * mask_image + (1 - mask_image)
    # Dictionary mapping image tensors to their folder names and file prefixes
    #breakpoint()
    images_dict = {
        'rgb_gt': (rgb_gt.cpu(), 'rgb_gt', False),  # Apply mask to RGB
        # 'env': (env.cpu(), 'env', False),  # Apply mask to RGB

        'normal': (normal_image.cpu(), 'normal_gt', False),
        'albedo': (albedo_image.cpu(), 'albedo_gt', False),

        'spec_light': (spec_light_image.cpu(), 'spec_gt', False),
        'diff_light': (diff_light_image.cpu(), 'diff_gt', False),

    }

    for folder_name, (image_tensor, file_prefix, is_grayscale) in images_dict.items():
        folder_path = os.path.join(root_folder, folder_name)
        os.makedirs(folder_path, exist_ok=True)
        if image_tensor.dim() == 4:  # Batch of images
            batch_size = image_tensor.size(0)
            for i in range(batch_size):
                img = image_tensor[i]  # Select individual image from batch
                file_path = os.path.join(folder_path, f'{file_prefix}_{ind}_{i}.png')
                if is_grayscale:
                    save_tensor_as_image(img, file_path, mean=[0.5], std=[0.5])
                else:
                    img = Image.fromarray(((img + 1) * 127.5).permute(1, 2, 0).numpy().astype(np.uint8))
                    img.save(file_path)

        else:  # Single image
            file_path = os.path.join(folder_path, f'{file_prefix}_{ind}.png')
            if is_grayscale:
                save_tensor_as_image(image_tensor, file_path, mean=[0.5], std=[0.5])
            else:
                img = Image.fromarray(((img + 1) * 127.5).permute(1, 2, 0).numpy().astype(np.uint8))
                img.save(file_path)

    # batch_folder = os.path.join(root_folder, "cam_data")
    # os.makedirs(batch_folder, exist_ok=True)
    
    # batch_size = mv.size(0)
    # for i in range(batch_size):
    #     mv_i = mv[i].cpu().numpy().tolist()
    #     mvp_i = mvp[i].cpu().numpy().tolist()
    #     campos_i = campos[i].cpu().numpy().tolist()
    #     data = {
    #         'mv': mv_i,
    #         'mvp': mvp_i,
    #         'campos': campos_i
    #     }
    #     file_path = os.path.join(batch_folder, f'{ind}_{i}.json')
...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, 
    controldec, args, accelerator, weight_dtype, step, test_dataloader, img_guidance_scale=0.0, compute_times=1, mask_guidance_scale=0.0):
...
# functional call
    ...
    for raw_batch in test_dataloader:
        ...
        # depth = batch['input_depths'].squeeze(1)[0]
        env_image = batch['input_env'].squeeze(1)
        
        batch['input_alphas'] = batch['input_alphas'].bool().float()
        masks_image = batch['input_alphas'].squeeze(1)
        ...
        materials = torch.tensor(batch['input_materials']).squeeze(1).to(masks_4_eval.device)
        
        material_for_gt = batch['input_materials']
        #breakpoint()
        
        #_, metallic_image, roughness_image = process_material_and_mask(material_for_gt, masks_image, images.device)
        normal_image = batch['input_normals'].squeeze(1)
        albedo_image = batch['input_albedos'].squeeze(1)
        
        prompts = ' '
        
        mv = batch['c2w_list_sample_2_save']
        mvp = batch['input_pose_list_sample_2_save']
        campos = batch['camera_pos_sample_2_save']

        images = images[0].unsqueeze(0)
        env_image = env_image[0].unsqueeze(0)
        spec_light_image = spec_light_image[0].unsqueeze(0)
        diff_light_image = diff_light_image[0].unsqueeze(0)
        normal_image = normal_image[0].unsqueeze(0)
        albedo_image = albedo_image[0].unsqueeze(0)
        masks_image = masks_image[0].unsqueeze(0)
        masks_4_eval = masks_4_eval[0].unsqueeze(0)
        materials = materials[0].unsqueeze(0)
        
        ############ dataset test ############
        vis_dataset(normalize(images), masks_image, normalize(env_image), normal_image, normalize(albedo_image), 
                     normalize(spec_light_image), 
                    normalize(diff_light_image), mv, mvp, campos, ind)
        ind += 1
        breakpoint()
        with torch.autocast("cuda"):
            ...
            for _ in range(compute_times):
                ...
                metallic = (material[0, :2].mean() + 1) * 127.5 * masks_image
                roughness = (material[0, 2:].mean() + 1) * 127.5 * masks_image
                #breakpoint()
                metallic_list.append(metallic.squeeze(0).permute(1, 2, 0).cpu().numpy())
                roughness_list.append(roughness.squeeze(0).permute(1, 2, 0).cpu().numpy())
                

            metallic = Image.fromarray(np.mean(metallic_list, axis=0).astype(np.uint8)) if compute_times > 1 else Image.fromarray(metallic_list[0].astype(np.uint8))
            roughness = Image.fromarray(np.mean(roughness_list, axis=0).astype(np.uint8)) if compute_times > 1 else Image.fromarray(roughness_list[0].astype(np.uint8))
            
            binary_mask = (masks_4_eval[:, 0:1, :, :].detach().cpu() > 0.5).squeeze(0).squeeze(0).numpy()     # [0, 1]
            not_binary_mask = (masks_4_eval[:, 0:1, :, :].detach().cpu() < 0.5).squeeze(0).squeeze(0).numpy()   # [0, 1]
            ...
            material_pred_num = np.array([metallic_pred_value, roughness_pred_value])

            # # metallic and roughness
            metallic_num, roughness_num = materials[:, 0].item(), materials[:, 1].item()
            material_num = np.array([metallic_num, roughness_num])
            # m_error_list.append(math.sqrt((metallic_pred_value - metallic_num)**2))
            # r_error_list.append(math.sqrt((roughness_pred_value - roughness_num)**2))
            ...
            re_rendered_rgb_image_4_vis = re_rendered_rgb_image
            ################ re-render ###################
            ...
            # re_render_images.append(re_rendered_rgb_image_4_vis[0])
            
            # save_relight_dir = os.path.join(args.output_dir, 'relight')
            # os.makedirs(save_relight_dir, exist_ok=True)
            # save_relight_1_dir = os.path.join(args.output_dir, 'relight_1')
            # os.makedirs(save_relight_1_dir, exist_ok=True)
            save_normal_dir = os.path.join(args.output_dir, 'normal')
            os.makedirs(save_normal_dir, exist_ok=True)            
            
            # save_normal_gt_dir = os.path.join(args.output_dir, 'normal_gt')
            # os.makedirs(save_normal_gt_dir, exist_ok=True)

            save_specular_dir = os.path.join(args.output_dir, 'specular')
            os.makedirs(save_specular_dir, exist_ok=True)

            # save_specular_gt_dir = os.path.join(args.output_dir, 'specular_gt')
            # os.makedirs(save_specular_gt_dir, exist_ok=True)

            save_diffuse_dir = os.path.join(args.output_dir, 'diffuse')
            os.makedirs(save_diffuse_dir, exist_ok=True)

            # save_diffuse_gt_dir = os.path.join(args.output_dir, 'diffuse_gt')
            # os.makedirs(save_diffuse_gt_dir, exist_ok=True)

            save_albedo_dir = os.path.join(args.output_dir, 'albedo')
            os.makedirs(save_albedo_dir, exist_ok=True)

            # save_albedo_gt_dir = os.path.join(args.output_dir, 'albedo_gt')
            # os.makedirs(save_albedo_gt_dir, exist_ok=True)
            ...
            normal[0].save(f'{save_normal_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            spec_light[0].save(f'{save_specular_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            diff_light[0].save(f'{save_diffuse_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            albedo[0].save(f'{save_albedo_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')


            # normal_image[0].save(f'{save_normal_gt_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            # spec_light[0].save(f'{save_specular_gt_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            # diff_light[0].save(f'{save_diffuse_gt_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            # albedo[0].save(f'{save_albedo_gt_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
            breakpoint()
            ####### documentation ########
    ...
    m_error_mean = sum(m_error_list) / len(m_error_list)
    r_error_mean = sum(r_error_list) / len(r_error_list)

    validation_scores_path = os.path.join(save_img_dir, 'validation_scores.txt')
    # Write the metrics along with the iteration number to a text file in save_img_dir
    with open(validation_scores_path, 'a') as f:
        f.write(f"Step {step}:\n")
        ...
        f.write(f"MSE Metallic: {m_error_mean:.4f}\n")
        f.write(f"MSE Roughness: {r_error_mean:.4f}\n")
    ...
    #         [formatted_metallics, formatted_metallics_gt, formatted_roughnesses, \
    #          formatted_roughnesses_gt, formatted_normals, formatted_normals_gt, formatted_albedos,  \
    #          formatted_albedos_gt, formatted_spec_lights,  formatted_spec_lights_gt, formatted_diff_lights, \
    #          formatted_diff_lights_gt, formatted_re_render_images, formatted_real_images], axis=1)
    ...
    #     #breakpoint()
    #     im.save(f'{save_img_dir}/{str(step)}_cfg{mask_guidance_scale}_{img_guidance_scale}.png')
        #breakpoint()
...
def main(args):
    ...
    test_dataset = ObjaverseData_relight(root_dir=root_dir, root_dir_new=root_dir_new, light_dir=light_dir)
